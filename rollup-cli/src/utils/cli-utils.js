...
async function withdrawOffChainTx(urlOperator, amount, walletJson, passphrase, tokenId, fee, nonce, nonceObject) {
    const walletRollup = await Wallet.fromEncryptedJson(walletJson, passphrase);
    return send(urlOperator, [Constants.exitAx, Constants.exitAy], amount, walletRollup, tokenId, fee, nonce, nonceObject);
}

async function crossChainTx(urlOperator, chainAccountIdx, amount, walletJson, passphrase, tokenId, fee, nonce, nonceObject) {
