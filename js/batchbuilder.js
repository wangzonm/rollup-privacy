...
module.exports = class BatchBuilder {
    ...
    async _addTx(tx) {
        ...
        // Find and set Idx
        let fromIdx = Constants.getMultiChainAccountIdx(tx.fromAx, tx.fromAy);
        if (fromIdx == null) {
            const hashFromIdx = utils.hashIdx(tx.coin, tx.fromAx, tx.fromAy);
            fromIdx = await this.dbState.get(hashFromIdx);
        }

        let toIdx = Constants.getMultiChainAccountIdx(tx.toAx, tx.toAy);
        if (toIdx === null) {
        ...
        let op2 = "INSERT";
        let isExit;
        let isCrossChainSend;
        let isCrossChainReceive;
        let newAccount = 0;
        ...
        if (!Constants.isMultiChainAccount(tx.fromIdx)) {
            ...
            isCrossChainReceive = false;
        } else {
            const {ax, ay, ethAddr} = Constants.getMultiChainAccount(tx.fromIdx);
            oldState1 = {
                amount: Scalar.e(0),
                coin: tx.coin,
                nonce: 0,
        ...
        if (tx.toIdx && (!Constants.isMultiChainAccount(tx.toIdx))) {
            ...
            isExit = false;
            isCrossChainSend = false;
            op2 = "UPDATE";
        } else if(tx.toIdx == Constants.exitAccountIdx) {
